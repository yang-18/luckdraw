...
def login(request):
    if request.method == "POST":
        ...
        if telephone is None or password is None:   # or len(telephone) == 0 or len(password) == 0
        ...
        if password == user.password:
            user.password = make_password(password)   # make_password(password, salt=None（指定时每次生成的密文相同）, hasher='default'（加密方法）)
            user.save()
